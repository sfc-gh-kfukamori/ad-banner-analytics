...
さらに **Snowflake Intelligence** として Cortex Agent（Cortex Analyst + Cortex Search + Web Search + Chart）を Snowsight UI から利用できるエージェント体験も構築済みです。
...
flowchart TB
    ...
    subgraph SI["Snowflake Intelligence (Snowsight UI)"]
        AGENT["Cortex Agent<br/>AD_BANNER_ADVISOR_AGENT<br/>(広告バナー分析アドバイザー)"]
    end
    ...
    subgraph Snowflake["Snowflake Data Platform"]
        ...
        WEB["Web Search"]
        CHART["data_to_chart<br/>(チャート自動生成)"]
    end
...
### 7. Snowflake Intelligence（Cortex Agent）

Snowsight UI から直接利用できる AI エージェント体験。Cortex Agent が 4 つのツールを自律的に使い分けて回答します。

| 項目 | 内容 |
|---|---|
| エージェント名 | `AD_BANNER_ADVISOR_AGENT`（表示名: 広告バナー分析アドバイザー） |
| ツール 1 | **Cortex Analyst** — Semantic View 経由でキャンペーン配信データを SQL 分析 |
| ツール 2 | **Cortex Search** — 社内ナレッジベース（過去分析レポート）を類似度検索 |
| ツール 3 | **Web Search** — 最新の広告業界トレンド・ベンチマーク情報を取得 |
| ツール 4 | **data_to_chart** — データ分析結果からチャートを自動生成 |
| サンプル質問 | CTR 最高バナー / CTA 色変更の成功事例 / CTR ベンチマーク / CVR 推移グラフ / デバイス別 CPA 比較 |

**アクセス方法:** Snowsight → AI & ML → Agents → 「広告バナー分析アドバイザー」を選択  
**裏側の仕組み:** Cortex Agent のオーケストレーション LLM がユーザーの質問を解釈し、適切なツールを選択・実行。複数ツールを組み合わせた回答（例: データ分析 + ナレッジ引用 + チャート生成）も可能。全てのクエリはユーザーの RBAC 権限に従って実行されます。
...
| **Cortex Agent** | 複数ツールを自律的にオーケストレーションする AI エージェント |
| **Snowflake Intelligence** | Cortex Agent を Snowsight UI から利用するエージェント体験 |
| **Web Search** | Cortex Agent 経由で最新の広告業界トレンドを取得 |
| **data_to_chart** | Cortex Agent 経由でデータ分析結果からチャートを自動生成 |
...
├── setup_snowflake_intelligence.sql # Snowflake Intelligence / Cortex Agent 定義
...
### 6. Snowflake Intelligence セットアップ

```sql
-- Cortex Agent の作成と Snowflake Intelligence への登録
-- setup_snowflake_intelligence.sql を Snowflake で実行
```

セットアップ後、Snowsight → **AI & ML** → **Agents** → 「広告バナー分析アドバイザー」からチャット形式で利用できます。
...
| データウェアハウス | Snowflake |
| AI/ML | Snowflake Cortex AI SQL 関数, Cortex Analyst, Cortex Search, Cortex Agent |
| エージェント | Snowflake Intelligence (Snowsight UI), Web Search, data_to_chart |
